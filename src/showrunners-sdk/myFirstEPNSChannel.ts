...
const channelKey = channelWalletsInfo.walletsKV['myFirstEPNSChannelPrivateKey_1']
...
export default class MyChannel {
  ...
  public async sendMessageToContract(simulate) {
    logger.info(`[${new Date(Date.now())}]-[My First EPNS Channel]- uploading payload and interacting with smart contract...`);
    const title = 'My First EPNS Channel';
    const message = `Hello World`;
    const payloadTitle = 'My First EPNS Channel';
    const payloadMsg = `Hello World`;
    const channelAddress = ethers.utils.computeAddress(channelWalletsInfo.walletsKV['myFirstEPNSChannelPrivateKey_1'])
